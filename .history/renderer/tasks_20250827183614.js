...
document.getElementById('printPendingBtn').addEventListener('click', () => {
  ...
  let html = `
    <html>
    <head>
      <title>Pending Tasks Report${descriptionTitle}</title>
      <style>
        body { font-family: Arial, sans-serif; }
        .scaled-report {
          transform: scale(0.5);
          transform-origin: top left;
          width: 25vw; /* Use 25% of viewport width for screen */
          margin: 0 auto;
        }
        @media print {
          body, html { background: #fff !important; }
          .scaled-report {
            width: 5cm;           /* About one fourth of A4 width */
            margin-left: auto;
            margin-right: auto;
            margin-top: 2cm;
          }
    ...
    <body>
      <div class="scaled-report">
        <h2>Pending Tasks Report${descriptionTitle}</h2>
        <table>
          <thead>
            <tr>
              <th>Task ID</th>
