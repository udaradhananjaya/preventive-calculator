...
// 2. Handle Enter key in title and description
...
descriptionInput.addEventListener('keydown', async (e) => {
  if (e.key === 'Enter' && !e.shiftKey && descriptionInput.value.trim() !== '') {
...
async function handleFormSubmit() {
  ...
  try {
    ...
    if (err.type === 'DUPLICATE_TASK') {
      const { title, description, createdAt } = err.existing;
      showToast(
        `A task with this title already exists for ${new Date(createdAt).getFullYear()}: 
        "${title}" — ${description} (Created: ${new Date(createdAt).toLocaleDateString()})`,
        'danger'
      );
    } else {
      console.error(err);
