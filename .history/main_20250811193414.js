...
ipcMain.handle('add-task', async (event, data) => {
  try {
    ...
    if (err.name === 'SequelizeUniqueConstraintError') {
      // Extract the year from the createdAt being inserted
      const year = new Date(data.createdAt).getFullYear();

      // Find the existing record with same title and year
        ...
        where: {
          ...
          [Op.and]: [
            sequelize.where(
              ...
              year.toString()  // must be string because strftime returns string
            )
          ]
        }
      });
      ...
      if (existingTask) {
        // Throw a custom error object to the frontend
        throw {
          type: 'DUPLICATE_TASK',
          message: 'Duplicate task found for this year.',
          existing: {
