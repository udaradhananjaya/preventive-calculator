...
app.whenReady().then(async () => {
  try {
    ...
    await syncOncePerDay(); // <-- Add this line
...
const SYNC_DATE_FILE = path.join(app.getPath('userData'), 'last_sync_date.txt');

async function syncOncePerDay() {
  let lastSyncDate = null;
  if (fs.existsSync(SYNC_DATE_FILE)) {
    lastSyncDate = fs.readFileSync(SYNC_DATE_FILE, 'utf-8').trim();
  }
  const today = new Date().toISOString().slice(0, 10); // 'YYYY-MM-DD'
  if (lastSyncDate !== today) {
    await syncTasksToFirebase();
    fs.writeFileSync(SYNC_DATE_FILE, today);
    console.log('Tasks synced to Firebase for', today);
  } else {
    console.log('Tasks already synced today:', today);
  }
}
...
// Check if a task title exists
ipcMain.handle('task-title-exists', async (event, title) => {
  const count = await Task.count({ where: { title } });
  return count > 0;
});
