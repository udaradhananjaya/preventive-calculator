...
function displayTasks(tasksToRender) {
  ...
  // If not searching, show only the latest 100 tasks
...
document.getElementById('confirmMarkAllBtn').addEventListener('click', () => {
    ...
    checkboxes.forEach(cb => {
        // Only mark as completed if not already checked and not detained
        // The detained status is reflected by the disabled property (see displayTasks logic)
...
document.getElementById('printPendingBtn').addEventListener('click', () => {
  ...
  const descriptionTitle = descriptions.length > 0 ? ` - ${descriptions.join(', ')}` : '';
    ...
    <head>
      ...
      <style>
        ...
        .scaled-report {
          ...
          width: 8cm; /* Fixed width for print and screen */
  ...
  html += `
    ...
    <script>
      ...
      // Fallback: close after 1.5 seconds if afterprint doesn't fire
...
if (searchDateInput) {
  searchDateInput.addEventListener('click', function () {
    this.showPicker && this.showPicker(); // Modern browsers
    // For older browsers, fallback to focus
    this.focus();
  });
}
...
document.addEventListener('keydown', (e) => {
  if (e.ctrlKey && e.key.toLowerCase() === 'p') {
    // Only trigger if the list is filtered (search input is not empty)
    if (searchInput.value.trim() !== '') {
      e.preventDefault();
      document.getElementById('printPendingBtn').click();
    }
  }
});