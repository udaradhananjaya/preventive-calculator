...
window.api.onSetTitleField( async (value) => {
  titleInput.value = value;
  // Check for duplicate title
  const exists = await window.api.taskTitleExists(value);
  if (exists) {
    showToast('A task with this ID already exists.', 'danger');
    titleInput.select();
    return;
  } else {
...
titleInput.addEventListener('keydown', async (e) => {
  if (e.key === 'Enter' && titleInput.value.trim() !== '') {
    ...
    // Check for duplicate title
    const exists = await window.api.taskTitleExists(title);
    if (exists) {
      showToast('A task with this ID already exists.', 'danger');
      titleInput.select();
      return;
...
const searchDateInput = document.getElementById('createdAt');
if (searchDateInput) {
